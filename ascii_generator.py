...
def create_ascii_frame(ascii_str, frame_index, total_frames, font, output_path, color_data=None):
    ...
    if frame_index >= total_frames:
        ...
        chars_to_display = min(int(round((frame_index + 1) * chars_per_frame)), total_chars)    # Get the indices we need for this frame
    ...
    if not hasattr(create_ascii_frame, 'index_mapping'):
        ...
        if len(create_ascii_frame.index_mapping) > 0:
            ...
            create_ascii_frame.index_values = torch.tensor(values, device=DEVICE, dtype=torch.long)    # Create the character mask using GPU acceleration
    char_mask = {}
    ...
    if OPTIMIZATION_LEVEL == 0:
        ...
        # Free memory
        del img_np      # Clear GPU memory only if needed (not on each frame)
